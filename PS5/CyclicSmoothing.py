...
def smooth(path, weight_data = 0.1, weight_smooth = 0.1, tolerance = 0.00001):
    ...
    while change >= tolerance:
        ...
        for i in range(len(newpath)):
            new_zero = newpath[i][0] + weight_data * (path[i][0] - newpath[i][0]) + weight_smooth * (newpath[(i+1) %len(newpath)][0] + newpath[(i-1)%len(newpath)][0] - (2 * newpath[i][0]))
            new_one = newpath[i][1] + weight_data * (path[i][1] - newpath[i][1]) + weight_smooth * (newpath[(i+1)%len(newpath)][1] + newpath[(i-1)%len(newpath)][1] - (2 * newpath[i][1]))
            change += abs(newpath[i][0] - new_zero) + abs(newpath[i][1] - new_one)
            newpath[i][0] = new_zero
            newpath[i][1] = new_one
