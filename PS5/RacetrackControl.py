...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        # the track will have a length x = 4 * radius and a height y = 2 * radius

        x, y = self.x, self.y
        if x < radius:
            # going "upwards" on the left side of the circular track
            dx = x - radius
            dy = y - radius
            eucdist = sqrt(pow(dx, 2) + pow(dy, 2))
            cte = eucdist - radius
        elif x > 3 * radius:
            # going "downwards" on the right side of the circular track
            dx = x - 3 * radius
            dy = y - radius
            eucdist = sqrt(pow(dx, 2) + pow(dy, 2))
            cte = eucdist - radius
        elif y > radius:
            # going left on the top of the track
            cte = y - 2 * radius
        else:
            # going right on the bottom of the track
            cte = -y
