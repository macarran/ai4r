...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    # Dimension is now a single path plus the number of landmarks. Multiply it by two, because we are on 2D now.
    dim = 2 * (1 + num_landmarks)

    # Constraint info vector creation and initialization.
    Omega = matrix()
    Omega.zero(dim, dim)
    ...
    Xi = matrix()
    Xi.zero(dim, 1)
    # As described in the lecture we start at the center of the world.
    Xi.value[0][0] = world_size / 2.0
    Xi.value[1][0] = world_size / 2.0
    ...
    for k in range(len(data)):
        ...
        for i in range(len(measurement)):
            # m = index of the landmark's coordinate in the matrix/vector
        ...
        # prepare the list to expand. We are going to use indexes 2 and 3 to add X and Y for the new pose, which means
        # that we are going to want to keep 0,1 as is and then insert the values for the new pose after them, followed
        # any amount of trailing locations.
        list = [0, 1] + range(4, dim + 2)
        Omega = Omega.expand(dim + 2, dim + 2, list, list)
        Xi = Xi.expand(dim + 2, 1, list, [0])

        # Process motion
        # for each two variables being tied together... 2 * 2D = 4
        for b in range(4):
        ...
        # Now it's time to remove the stale pose and calculate A, B and C.
        takeList = range(2, len(Omega.value))
        A = Omega.take([0, 1], takeList)
        B = Omega.take([0, 1])
        C = Xi.take([0, 1], [0])
        Omega = Omega.take(takeList) - (A.transpose() * B.inverse() * A)
        Xi = Xi.take(takeList, [0]) - (A.transpose() * B.inverse() * C)

    # compute best estimate
    mu = Omega.inverse() * Xi
    print '{}x{}'.format(Xi.dimx, Xi.dimy)
